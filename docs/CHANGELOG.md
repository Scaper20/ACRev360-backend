...
## 2026-09-25 — Onboarding switched from `username` to `email`; `address` added for 3 account types (`ACRev360-backend`)

**Ask:** the two items scoped as a handoff spec in the entry below (`docs/BACKEND_HANDOFF_ONBOARDING_EMAIL_ADDRESS.md`) — this entry is that spec actually built. Confirmed live: none of the 5 onboarding flows (Field Agent, Revenue Officer, Stakeholder, Ratepayer invite, Consultant manager) ever passed `email=` to `AppUser.objects.create_user(...)`, so every account onboarded through them got `AppUserManager._create_user`'s `@placeholder.acrev360.local` fallback and could never actually sign in with a real email, even though login itself (`AppTokenObtainPairSerializer`) is fully email-based.

**Changed — `username` → `email` on all 5 onboarding serializers:** `FieldAgentSerializer`, `RevenueOfficerSerializer`, `StakeholderSerializer`, `InviteRatepayerSerializer` (all required), and `SubConsultantSerializer`'s `manager_username`/`manager_email` (stays optional — onboarding a firm with no login yet is still valid). `username` is no longer client-supplied anywhere; it's derived server-side from the email's local-part (`apps/accounts/services.py::derive_username_from_email`, e.g. `jane.doe@x.com` → `jane.doe`), with a numeric suffix appended on collision. `AppUser.username` keeps `unique=True` and stays `USERNAME_FIELD` — unchanged, per the model's own docstring. A shared `validate_unique_email` field validator (`apps/accounts/validators.py`, `email__iexact` to match login's own case-insensitive lookup) was added to every one of the 5 flows — **this also fixed a live bug**: only Field Agent previously pre-checked uniqueness before `create_user()`; the other 4 hit the raw DB `unique=True` constraint on a duplicate and 500'd with zero client-facing detail (uncaught `IntegrityError`, not a DRF `APIException` — same failure shape already fixed once for `SubConsultant.contract_ref`).

**Added — `address` for Field Agent, Stakeholder, Revenue Officer:** Field Agent got a plain new column (`FieldAgent.address`, mirrors `Payer.address`'s shape exactly). Stakeholder and Revenue Officer are bare role-tagged `AppUser` rows with no dedicated model — rather than bolting `address` onto `AppUser` itself (would've put it on every account type, including council-staff logins with no use for it — the handoff spec's own "Option A"), gave each its own `OneToOneField` profile table (`StakeholderProfile`, `RevenueOfficerProfile`), room for future role-specific fields included. `StakeholderSerializer`/`RevenueOfficerSerializer` read/write `address` through `to_representation`/`perform_create` rather than a dotted `source=`, since `perform_create` already builds the `AppUser` (now plus the profile) by hand instead of calling `serializer.save()`. One migration (`apps/accounts/migrations/0008_...py`): `FieldAgent.address` + both new tables.

**Gotchas:**
- **The handoff spec recommended "Option A"** (address straight on `AppUser`) as the smaller change; this build went with "Option B" (dedicated profile models) per explicit instruction in the follow-up ask — don't "simplify" this back to Option A without checking that ask first, it was a deliberate tradeoff (future role-specific fields over migration size), not an oversight.
- **`AppUserManager._create_user`'s placeholder-email fallback is still there** — every onboarding flow now passes `email=` explicitly, but the fallback stays live for paths that bypass the serializers entirely (`createsuperuser`, the `seed_*` management commands) and still don't pass one.
- **`derive_username_from_email` isn't collision-atomic** — it precheck-loops (`AppUser.objects.filter(username=...).exists()`) the same way the `contract_ref` precheck elsewhere in this codebase does, not inside a DB-level lock. Two concurrent onboards for the same local-part could theoretically both compute the same suffix and have one lose to a raw `IntegrityError` on `username` (not `email`, which is precheck-validated by the serializer) — accepted as the same benign race every other precheck in this codebase already tolerates, not treated as a new bug class.
- **Frontend follow-up is a same-day field swap** (Username → Email on 5 forms; add Address to 3), already scoped in the handoff spec below — nothing further needed backend-side.
